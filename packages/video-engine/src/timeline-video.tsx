...
import {
  AccentBar, ActionList, AlertLayout, BlockSequence, CloseLayout, DarkShell, DefenseLayout,
  EventLayout, GlitchTitle, NarrativeText, PhaseLabel, Reveal, YearBadge,
} from "./components";
import type { VideoProps } from "./video-metadata";
import { closeTitleSize, eventTitleSize, introMotion, sceneImage, sceneLines, sceneText, titleScale, VideoScenes } from "./video-scenes";
  ...
  return (
    <DarkShell accent={accent} durationInFrames={d} variant="alert" bgSrc={sceneImage(scene, assetBaseUrl)} niche={document.niche} hookStyle={document.hookStyle} sceneKey="intro">
      <AlertLayout
        ...
        subtitle={<BlockSequence from={motion.subtitleFrom} durationInFrames={d - motion.subtitleFrom}><Reveal y={16} blurFrom={10}><NarrativeText text={sceneText(scene.content, "subtitle")} size={motion.subtitleSize} /></Reveal></BlockSequence>}
      />
    </DarkShell>
  );
};
...
/* ─── event ─── */
const EventScene = ({ document, scene, durationInFrames: d, assetBaseUrl }: SceneProps) => {
  const accent = sceneAccent(scene);
  return (
    <DarkShell accent={accent} durationInFrames={d} variant="body" bgSrc={sceneImage(scene, assetBaseUrl)} niche={document.niche} hookStyle={document.hookStyle} sceneKey={scene.id as "event1" | "event2" | "event3" | "event4"}>
      <EventLayout
        tag={<BlockSequence from={0} durationInFrames={d}><Reveal y={12} blurFrom={8}><PhaseLabel text={sceneText(scene.content, "event")} accent={accent[0]} /></Reveal></BlockSequence>}
        year={<BlockSequence from={8} durationInFrames={d - 8}><Reveal y={30} scaleFrom={0.85} blurFrom={22} durationInFrames={36}><YearBadge year={sceneText(scene.content, "year")} accent={accent} /></Reveal></BlockSequence>}
        headline={<BlockSequence from={20} durationInFrames={d - 20}><Reveal y={18} blurFrom={14} durationInFrames={30}><GlitchTitle text={sceneText(scene.content, "headline")} accent={accent} size={eventTitleSize(document.niche)} /></Reveal></BlockSequence>}
        impact={<BlockSequence from={54} durationInFrames={d - 54}><Reveal y={14} blurFrom={10}><NarrativeText text={sceneText(scene.content, "impact")} size={40} /></Reveal></BlockSequence>}
      />
    </DarkShell>
  );
};
...
/* ─── today ─── */
const TodayScene = ({ document, scene, durationInFrames: d, assetBaseUrl }: SceneProps) => {
  const accent = sceneAccent(scene);
  return (
    <DarkShell accent={accent} durationInFrames={d} variant="body" bgSrc={sceneImage(scene, assetBaseUrl)} niche={document.niche} hookStyle={document.hookStyle} sceneKey="today">
      <DefenseLayout
        tag={<BlockSequence from={0} durationInFrames={d}><Reveal y={12} blurFrom={8}><PhaseLabel text={sceneText(scene.content, "tag")} accent={accent[0]} /></Reveal></BlockSequence>}
...
const scenesByKind = { intro: IntroScene, event: EventScene, today: TodayScene, close: CloseScene } as const;

export function TimelineVideo({ document, assetBaseUrl }: VideoProps) {
  return (
    <VideoScenes
      document={document}
