...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await withDatabase(async (database) => await database.prepare("SELECT data_json FROM render_jobs WHERE id = ?").get(id) as { data_json: string } | undefined);
  ...
  if (input.action === "retry") {
    ...
    const parsed = renderJobSchema.safeParse({ ...job, status: "queued", progress: 0, outputAssetId: null, completedAt: null, error: null, log: [] });
    if (!parsed.success) return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
    const result = await withDatabase(async (database) => await database.prepare("UPDATE render_jobs SET data_json = ? WHERE id = ? AND (data_json::jsonb->>'status') IN ('failed', 'cancelled')").run(JSON.stringify(parsed.data), id) as { changes: number });
    ...
    const latest = await withDatabase(async (database) => await database.prepare("SELECT data_json FROM render_jobs WHERE id = ?").get(id) as { data_json: string } | undefined);
