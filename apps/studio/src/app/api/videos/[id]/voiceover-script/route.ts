...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await withDatabase(async (database) => await database.prepare("SELECT document_json, revision FROM content_items WHERE id = ? AND type = 'video' AND archived_at IS NULL").get(id) as { document_json: string; revision: number } | undefined);
