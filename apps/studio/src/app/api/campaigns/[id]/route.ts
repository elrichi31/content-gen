...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withDatabase((database) => {
    ...
    const content = database.prepare("SELECT id, type, revision, updated_at FROM content_items WHERE campaign_id = ? AND archived_at IS NULL ORDER BY updated_at DESC").all(id) as { id: string; type: string; revision: number; updated_at: string }[];
    ...
    return { ...JSON.parse(campaign.data_json), pieces: content.map((item) => { const render = renderQuery.get(item.id) as { data_json: string } | undefined; const renderStatus = render ? JSON.parse(render.data_json).status as string : null; return { id: item.id, type: item.type, status: campaignPieceStatus({ revision: item.revision, hasExport: Boolean(exportQuery.get(item.id)), renderStatus }), updatedAt: item.updated_at }; }) };
  });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await withDatabase((database) => database.prepare("SELECT data_json FROM campaigns WHERE id = ?").get(id) as { data_json: string } | undefined);
