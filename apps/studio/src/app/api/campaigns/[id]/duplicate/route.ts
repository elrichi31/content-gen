...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await withDatabase(async (database) => await database.prepare("SELECT data_json FROM campaigns WHERE id = ? AND archived_at IS NULL").get(id) as { data_json: string } | undefined);
