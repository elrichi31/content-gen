...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await withDatabase(async (database) => {
    ...
    const pieces = (await database.prepare("SELECT id, type, document_json FROM content_items WHERE campaign_id = ? AND archived_at IS NULL ORDER BY id").all(id) as { id: string; type: "carousel" | "ad" | "video"; document_json: string }[]).map((row) => ({ id: row.id, type: row.type, document: JSON.parse(row.document_json).document.data }));
    const exports = await database.prepare("SELECT export.data_json AS export_json, asset.data_json AS asset_json, content.type FROM exports export JOIN assets asset ON asset.id = export.asset_id JOIN content_items content ON content.id = export.content_item_id WHERE content.campaign_id = ? AND content.archived_at IS NULL ORDER BY content.type, content.id, export.created_at").all(id) as { export_json: string; asset_json: string; type: string }[];
