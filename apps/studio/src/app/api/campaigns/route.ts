...
export async function GET() {
  const campaigns = await withDatabase((database) => (database.prepare("SELECT data_json FROM campaigns WHERE archived_at IS NULL ORDER BY updated_at DESC").all() as { data_json: string }[]).map((row) => JSON.parse(row.data_json)));
